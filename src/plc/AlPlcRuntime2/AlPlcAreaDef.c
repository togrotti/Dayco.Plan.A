...
#define PLC_S_SOURCE_CODE_AREA		1				/*	Target supports the storage of the source code in a dedicated area */
...
#define ALPLC_TARGET_ID		"AxX_"IDENT_SVERSION_MAJOR"p"IDENT_SVERSION_MINOR
...
*/
#define ALPLCINIT                   InitPlcRuntime          /*  Init oft the runtime, prototype: bool_t ALPLCINIT(void)   */
#define ALPLCLOAD                   LoadPlc                 /*  Load code into exec memory, prototype: bool_t ALPLCLOAD(void)   */
#define ALPLCMANAGE                 ManagePLCState          /*  Manage of runtime state, prototype: void ALPLCMANAGE(void)  */
...
*/
#define PLC_CODE_AREA1              m_PlcCodeArea1      	/*  Required - Address of main or first hot swap code area */
...
#define PLC_RETDATA_AREA            psPlcRetMgrData.bData   /*  Address of PLC retain data area */

#define PLC_CODE_SIZE               PLC_RT_CODE_SIZE        /*  Required - Size of code area(s) */
#define PLC_DATA_SIZE               PLC_AUTO_VARS_SIZE      /*  Required - Size of data area */
#define PLC_RETDATA_SIZE            PLCRETAINMGR_SIZE       /*  Size of PLC retain data area */
...
*/
#define PLC_R_COMMAND				uwPlcRegCommand			/*	LogicLab request for target reset/reload of PLC */
#define PLC_R_DOWNLADDR				ulPlcRegDownloadAddress	/*	Address for the next download */
#define PLC_R_INFOAREA				ulPlcRegInfoArea		/*	Address of runtime info area */
#define PLC_R_DOWNL_RESP			uwPlcRegDownloadResponse /*	Target response for a LogicLab request for download acknowledge */
#define PLC_R_APPL_ID				ulPlcRegApplID			/*	Application ID for the currently running PLC */
#define PLC_R_MEMO_ID				ulPlcRegMemoID			/*	Firmware memory ID */
#define PLC_R_FULLMEMO_ID			ulPlcRegFullMemoID		/*	Firmware memory ID with db and function check */
#define PLC_R_COMMAND_RESPONSE		ulPlcRegCommandResponse	/*	Response to LogicLab requests */
...
//#define PLC_F_ZEROAUXDATA_AREA			xmemset(PLC_AUXDATA_AREA, 0, PLC_AUXDATA_SIZE)	/*	Function call for reset data area: void funct( uint8_t address, uint32_t size_in_bytes ) */
...
#define PLC_F_INIT_LOAD				InvalidateCache()
...
*/
#define PLC_F_WRITE(dat,off,len)	WrPlc( (dat), (off), (len))	/* Function used to save PLC code into permanent memory */
#define PLC_F_READ(dat,off,len)		RdPlc( (dat), (off), (len))	/* Function used to load PLC code from permanent memory */
#define PLC_F_READ_INIT				RdPlcInit()					/* Function used to initialize permanent memory read */
#define PLC_F_READ_END				RdPlcEnd()					/* Function used to terminate permanent memory read */
#define PLC_F_WRITE_INIT			WrPlcInit()					/* Function used to initialize permanent memory writing */
#define PLC_F_WRITE_END				WrPlcEnd()					/* Function used to terminate permanent memory writing */
